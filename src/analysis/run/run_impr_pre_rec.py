...
def setup_arg_parser():
    ...
    parser.add_argument("--infer_latents", action="store_true",
                        help="Enable latent inference before calculating metrics")
    parser.add_argument("--device", type=str, default="cuda",
                        help="Device used for inference and metric calculation (e.g 'cpu' or 'cuda')")
...
def main():
    ...
    model = load_model_with_checkpoint(model_name, os.path.join(
        MODEL_DIR,
        "final_cvae_5_classes_ch16_kld50_base_20250722_072739_best"
    ))
    if is_infer_latents:
