...
from .bernoulli_diffusion import BernoulliDiffusionBeatGans
from .gaussian_diffusion import GaussianDiffusionBeatGans
...
@dataclass
class SpacedDiffusionBeatGansConfig(DiffusionBeatGansConfig):
    use_timesteps: Tuple[int] = None

    def make_sampler(self):
        if self.noise_type == NoiseType.gaussian:
            return GaussianSpacedDiffusionBeatGans(self)
        elif self.noise_type == NoiseType.xor:
            return BernoulliSpacedDiffusionBeatGans(self)
        else:
            raise NotImplementedError


class SpacedDiffusionBeatGans(DiffusionBeatGans, ABC):
    """
    ...
    def __init__(self, conf: SpacedDiffusionBeatGansConfig):
        ...
        if self.conf.noise_type == NoiseType.gaussian:
            ...
            base_diffusion = BernoulliDiffusionBeatGans(conf)
        else:
            raise NotImplementedError(f"The given noise type {self.conf.noise_type} is not implemented yet.")
...
class GaussianSpacedDiffusionBeatGans(SpacedDiffusionBeatGans, GaussianDiffusionBeatGans):
    def __init__(self, conf: SpacedDiffusionBeatGansConfig):
        super().__init__(conf)


class BernoulliSpacedDiffusionBeatGans(SpacedDiffusionBeatGans, BernoulliDiffusionBeatGans):
    def __init__(self, conf: SpacedDiffusionBeatGansConfig):
        super().__init__(conf)
