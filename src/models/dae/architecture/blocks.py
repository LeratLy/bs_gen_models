...
from src.models.dae.architecture.nn import (avg_pool_nd, conv_nd, linear, normalization,
...
@dataclass
class ResBlockConfig(ConfigFunctionalities):
    """
    Configuration for a Residual block containing a normalization layer which is applied to the input to have zero mean and variance. It then passes the normalized input in the activation function and applies a convolution

    :param channels: The number of input channels of the ResBlock, determining the depth of the input feature maps
    :type channels: int
    :param emb_channels: The style_channels or time embedding channels (time embedding channels provide info about provide info about the current timestep of the ResBlock (crucial for diffusion model))
    :type emb_channels: int
    :param dropout: Whether dropout should be used in the residual block., default to False
    :type dropout: bool
    :param out_channels: Output channels of the residual block determined by ``ch`` and ``ch_mult``, default to ``channels``
    :type out_channels: int
    :param use_condition: Whether the ResBlock should be conditioned with time (and encoder's output), default to True
    :type use_condition: bool
    :param use_conv: Whether to use 3x3 conv for skip path when the channels aren't matched to ensure matching dimensionality before addition, default to False
    :type use_conv: bool
    :param dims: Dimensionality of the convolution, default to 3 (using 3D convolutions)
    :param use_checkpoint: Whether gradient checkpoints should be used for memory efficiency, default to False
    :type use_checkpoint: bool
    :param up: An up-sampling ResBlock, default to False
    :type up: bool
    :param down: A down-sampling ResBlock, default to False
    :type down: bool
    :param two_cond: Whether to condition with both time & encoder's output (here the semantic encoder is meant), default to False
    :type two_cond: bool
    :param cond_emb_channels: Number of encoders' output channels, default to ``emb_channels``
    :type cond_emb_channels: int
    :param has_lateral: Whether the ResBlock receives a lateral (skip) connection from the encoder, default to False (suggested)
    :type has_lateral: bool
    :param lateral_channels: Number of channels in lateral connection, default to None
    :type lateral_channels: bool
    :param use_zero_module: Whether the convolution should be initialized with zero weights, default to True (seems to help learning)
    :type use_zero_module: bool
...
class ResBlock(TimestepBlock):
    ...
    def _forward(
        ...
        if self.conf.use_condition:
            ...
            # this happens with autoencoder and setting the time_at
            if emb is not None:
                # time embedding t_s
            ...
            if self.conf.two_cond:
                ...
                if cond is None:
                    ...
                    # z_s conditional embedding of semantic subcode
...
def apply_conditions(
    ...
    if two_cond:
        ...
        scale_shifts = [emb]
        if cond is not None:
            scale_shifts.append(cond)

    # support scale, shift or shift only
    # I guess one scale_shift per feature map
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dims == 3:
            # NOTE: use this if only two dimensions should be scaled
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.use_conv = use_conv
        self.dims = dims
        stride = 2 if dims != 3 else (2, 2, 2)  # NOTE: before it was (1, 2, 2) and it was only applied on 2 dimensions
        if use_conv:
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_efficient_attention:
            self.attention = EfficientAttention(self.num_heads)
...
class EfficientAttention(nn.Module):
    """
    TODO LeratLy (05.05.2025): softmax missing, not sure if correct test and work trough equations again
    Attention after Shen et al. (2021) https://github.com/cmsflash/efficient-attention/tree/master
    """
    ...
    def forward(self, qkv):
        ...
        attended_values = []
        # iterate over heads and perform attention per head
        for i in range(self.n_heads):
            q_i = q[:, i * ch: (i + 1) * ch, :]
            k_i = k[:, i * ch: (i + 1) * ch, :]
            v_i = v[:, i * ch: (i + 1) * ch, :]

            context = (k_i * scale) @ v_i.transpose(1, 2)
            attended_value = (context.transpose(1, 2) @ (q_i * scale))
            attended_values.append(attended_value)

        aggregated_values = th.cat(attended_values, dim=1)
