...
def target_embedding(targets, num_classes, dim):
    """
    Create target embedding
    """
    embedding = nn.Embedding(num_classes, dim).to(targets.device)
...
def torch_checkpoint(func, args, flag, preserve_rng_state=False):
    ...
    if flag:
        return torch.utils.checkpoint.checkpoint(
            func, *args, preserve_rng_state=preserve_rng_state, use_reentrant=False)
    else:
        return func(*args)
